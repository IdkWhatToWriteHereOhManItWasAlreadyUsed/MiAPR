            ...
            try
            {
                ...
                if (IsNumberCorrect(firstNumber, secondNumber))
                {
                    ...
                    for (int i = 0; i < NumberOfTests; i++)
                    {
                        ...
                        graphicsSecond.FillRectangle(Brushes.Blue, i,
                            pictureBox.Height -
                            (int) (probabilityDensityForSecondRandomVariables[i] * secondNumber * 150000), 3, 3);
                    }

                    double probFalseAlarm = CalculateProbabilitiesOfFalseAlarms(firstNumber, secondNumber, firstMathExpectation,
                            secondMathExpectation, firstStandardDeviation, secondStandardDeviation);
                    double probMissError = CalculateProbabilitiesOfMissingErrors(firstNumber, secondNumber, firstMathExpectation,
                            secondMathExpectation, firstStandardDeviation, secondStandardDeviation);



                    tbFalseAlarmProbability.Text = Convert.ToString(probFalseAlarm);


                    tbProbabilityOfMissingErrorDetection.Text = Convert.ToString(probMissError);


                    double a = probFalseAlarm +   probMissError;


                    tbProbabilityOfTotalClassificationError.Text = Convert.ToString(a );
                }
                else
                {
